...
import { AdminGiveawayPanel } from "@/app/staff/AdminGiveaway";
...
export type Dashboard = {
  ...
  products: Array<{ id: string; category_id: string; name: string; description: string; product_type: "pizza" | "simple" | "bundle" | "configurable"; image_url: string | null; base_price_cents: number; active: number; sold_out: number; pickup_eligible: number; delivery_eligible: number; taxable: number; setup_required: number; kitchen_label: string; configuration: Record<string, unknown>; display_order: number }>;
  toppings: Array<{ id: string; name: string; kitchen_label: string; is_meat: number; active: number }>;
  ...
  /** The Thanksgiving Giveaway while it is running, for the till's hint. */
...
// placement, extra cheese, deal/wing modifier selections, per-item and order
  ...
  return <div className="print-root">
    ...
    {items.map((item, index) => {
      ...
      return <div className="pt-item" key={String(item.id ?? index)}>
        ...
        {/* Extra cheese and anything the customer asked us to leave off.
...
export function LegacyMenuPanel({ dashboard, onSaved }: { dashboard: Dashboard; onSaved: () => Promise<void> }) {
  const [name, setName] = useState(""); const [label, setLabel] = useState(""); const [meat, setMeat] = useState(false); const [message, setMessage] = useState("");
  const submit = async (event: FormEvent) => { event.preventDefault(); const response = await fetch("/api/admin/config", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "topping.upsert", name, kitchenLabel: label, isMeat: meat, active: true }) }); const result = await response.json(); setMessage(response.ok ? "Topping added to the live menu." : result.error); if (response.ok) { setName(""); setLabel(""); await onSaved(); } };
